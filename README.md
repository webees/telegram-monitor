...
- **智能改写** — 转发前清理广告、识别主题，并按页面里的追加模板拼接内容
...
启用智能改写后，程序会调用 OpenAI 兼容接口清理正文、识别主题，并使用配置向导中的“追加内容模板”生成最终文案。如果 OpenAI key 用完、接口不可用或没有返回有效改写内容，这条消息会标记为失败并进入转发列表，不会用原文兜底转发。
...
| 智能改写失败 | OPENAI_API_KEY、额度、接口连通性；失败项可在转发列表重试 |
